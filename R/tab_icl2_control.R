...
#' @param ... Additional named arguments passed directly to the TabICL2 Python
...
#' @return A list with extra class `"control_tab_icl2"` that has named elements
#' for each of the argument values.
#' @references
#' \url{https://github.com/PriorLabs/TabICL2/blob/main/src/tabpfn/classifier.py},
#' \url{https://github.com/PriorLabs/TabICL2/blob/main/src/tabpfn/regressor.py}
#' @examples
#' control_tab_icl2()
#' @export
control_tab_icl2 <- function(
  ...
  class(args) <- "control_tab_icl2"
  args
}

#' @export
print.control_tab_icl2 <- function(x, ...) {
  defaults <- control_tab_icl2()
  ...
  cli_inform("control object for {.fn tab_icl2}")
  if (any(non_default)) {
    cat("\n")
