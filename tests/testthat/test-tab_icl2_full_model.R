...
test_that("TabICLv2 forward pass basic regression", {
  model <- TabICLv2(max_classes = 0L, num_quantiles = 3L)
  model$eval()
...
test_that("TabICLv2 handles single batch", {
...
test_that("TabICLv2 handles single feature column", {
  model <- TabICLv2(max_classes = 3L, col_feature_group_size = 1L)
  model$eval()
  x <- torch_randn(2L, 8L, 1L)
  y <- torch_randint(1, 4L, size = c(2L, 5L))
  output <- model(x, y)
  expect_tensor_shape(output, c(2L, 3L, 3L))
})

test_that("TabICLv2 handles all training no test", {
  model <- TabICLv2(max_classes = 4L)
  model$eval()
  x <- torch_randn(1L, 10L, 3L)
  y <- torch_randint(1, 5L, size = c(1L, 10L))
  output <- model(x, y)
  expect_tensor_shape(output, c(1L, 0L, 4L))
})

test_that("TabICLv2 output dtype matches input", {
  model <- TabICLv2(max_classes = 5L)
  model$eval()
  x_f32 <- torch_randn(2L, 12L, 3L, dtype = torch_float())
  y <- torch_randint(1, 6L, size = c(2L, 8L))
  output <- model(x_f32, y)
  expect_tensor_dtype(output, torch_float())
})

test_that("TabICLv2 gradient flow training mode", {
  model <- TabICLv2(max_classes = 3L)
  model$train()
  x <- torch_randn(2L, 10L, 4L, requires_grad = TRUE)
  y <- torch_randint(1, 4L, size = c(2L, 6L))
  output <- model(x, y)
  loss <- output$sum()
  loss$backward()
...
test_that("ClassEmbedding forward handles multi-label (2D labels)", {
  emb <- ClassEmbedding(num_embeddings = 5L, embedding_dim = 8L)
  y <- torch_randint(1, 6L, size = c(2L, 3L, 1L), dtype = torch_long())
  output <- emb(y)
...
test_that("TabICLv2 standardization uses training subset only", {
  model <- TabICLv2(max_classes = 3L)
  x <- torch_randn(1L, 10L, 4L)
  x[, 1:6, ..] <- x[, 1:6, ..] * 10 + 5
...
test_that("TabICLv2 model eval mode disables gradients", {
  ...
  y <- torch_randint(1, 4L, size = c(1L, 5L))
  with_no_grad({
    output <- model(x, y)
  })
...
test_that("TabICLv2 output is finite for random input", {
  ...
  y <- torch_randint(1, 11L, size = c(3L, 20L))
  output <- model(x, y)
  expect_true(all(is.finite(as_array(output))))
})

test_that("TabICLv2 output varies with input", {
  model <- TabICLv2(max_classes = 5L)
  model$eval()
  x1 <- torch_randn(1L, 12L, 4L)
  x2 <- x1 + 0.1
  y <- torch_randint(1, 6L, size = c(1L, 8L))
  out1 <- model(x1, y)
  out2 <- model(x2, y)
  expect_false(identical(as_array(out1), as_array(out2)))
})

test_that("TabICLv2 reproducibility with fixed seed", {
  torch::with_torch_manual_seed({
    model1 <- TabICLv2(max_classes = 3L)
    x1 <- torch_randn(1L, 10L, 3L)
    y1 <- torch_randint(1, 4L, size = c(1L, 6L))
    out1 <- model1(x1, y1)
  }, seed = 1792)
  torch::with_torch_manual_seed({
    model2 <- TabICLv2(max_classes = 3L)
    x2 <- torch_randn(1L, 10L, 3L)
    y2 <- torch_randint(1, 4L, size = c(1L, 6L))
    out2 <- model2(x2, y2)
  }, seed = 1792)
...
test_that("TabICLv2 load_state_dict restores weights", {
  model1 <- TabICLv2(max_classes = 3L)
  model1$eval()
